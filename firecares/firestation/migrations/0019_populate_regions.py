...
class Migration(DataMigration):
    def forwards(self, orm):
        ...
        regions = []
        with open(os.path.join(os.path.dirname(os.path.dirname(__file__)), 'data/nfpa-regions.csv'), 'r') as csvfile:
            rdr = csv.DictReader(csvfile)
            for row in rdr:
                regions.append(row)

        for f in orm.FireDepartment.objects.all():
            region = filter(lambda x: x['State'] == f.state, regions)[0]['Region']
            if region == '':
                f.region = None
            else:
                f.region = region
