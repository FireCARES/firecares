...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding field 'FireDepartment.temp_geom'
        db.add_column(u'firestation_firedepartment', 'temp_geom',
                      self.gf('django.contrib.gis.db.models.fields.MultiPolygonField')(null=True, blank=True),
                      keep_default=False)

        for f in orm.FireDepartment.objects.filter(geom__isnull=False):
            f.temp_geom = MultiPolygon(f.geom)
            f.save()


    def backwards(self, orm):
        # Deleting field 'FireDepartment.temp_geom'
        db.delete_column(u'firestation_firedepartment', 'temp_geom')
